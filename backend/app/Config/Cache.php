...
class Cache extends BaseConfig
{
    ...
    /**
     * Override handler from environment: set cache.handler=redis in .env to enable Redis
     */
    public function __construct()
    {
        parent::__construct();

        $envHandler = env('cache.handler');
        if ($envHandler && array_key_exists($envHandler, $this->validHandlers)) {
            $this->handler = $envHandler;
        }

        // Allow Redis config from environment
        $redisHost = env('cache.redis.host');
        if ($redisHost) {
            $this->redis['host'] = $redisHost;
        }
        $redisPort = env('cache.redis.port');
        if ($redisPort) {
            $this->redis['port'] = (int)$redisPort;
        }
        $redisPassword = env('cache.redis.password');
        if ($redisPassword) {
