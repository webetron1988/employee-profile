(function($) {
    ...
    function OrgChart($container, opts){
        ...
        this.draw = function(){
            ...
            $container.find('.node-competency').click(function(){
                if(self.opts.onClickNode !== null){
                    self.opts.onClickNode(nodes[$(this).attr('node-id')]);
                }
            });
             //   double click add child
            $(".node-competency").dblclick(function(){
            ...
            //   right click open form
              $(".node-competency").on("contextmenu", function(e) {
                e.preventDefault();
    ...
    function Node(data){
        ...
        this.formatNode = function(opts){
            ...
            if(typeof data.name !== 'undefined'){
                nameString = '<div class="'+self.data.parentClass+' structure-tile flex-column"><div class="text-center py-5 px-3"><span class="node-competency-line '+self.data.nodeBorderLeft+'"></span><h2 class="m-auto font-weight-normal">'+self.data.name+' <span class="align-self-center '+self.data.labelClass+'">'+self.data.label+'</span></h2><div class="competency-chart-actions d-flex justify-content-center"><div class="align-self-center mr-2"><btn class="btn btn-light-primary" data-toggle="modal" data-target="#add-competency-modal"><i class="la la-plus"></i></button></div><div class="align-self-center"><btn class="btn btn-light-primary" data-toggle="modal" data-target="#link-competency-modal" id="open-link-competency"><i class="la la-link"></i></button></div></div></div>';
            ...
            if(opts.showControls){
                var buttonsHtml = '<div class="org-chart-actions"><div class="hideChild btn p-0 mt-2"><svg class="d-none" width="9" height="9" viewBox="0 0 9 9" fill="none" xmlns="http://www.w3.org/2000/svg"> <path d="M8.09668 4.54834L1 4.54834" stroke="#B4BDDA" stroke-width="1.1" stroke-linecap="round" stroke-linejoin="round"/> <path d="M4.54785 8.09668L4.54785 1" stroke="#B4BDDA" stroke-width="1.1" stroke-linecap="round" stroke-linejoin="round"/> </svg></div> </div>';
            }
            else{
                buttonsHtml = '';
            }
            return "<div class='node-competency' node-id='"+this.data.id+"'>"+nameString+descString+buttonsHtml+"</div>";
        }
    }
})(jQuery);
